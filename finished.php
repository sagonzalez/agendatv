...
<body>
            ...
            <div id="content-right" class="col-md-9"> <!--all section of data-->
                ...
                <?php
                  if(isset($_GET['search'])){
                    $result = getFinish($_SESSION['_id'],$_SESSION['type'],$_GET['search']);
                  }else{
                    $result = getFinish($_SESSION['_id'],$_SESSION['type'],"");
                  }
                  echo getFinished($result);
                ?>  
                </div>
            </div>
        </div>
    ...
    <?php
      ...
      if(isset($_GET['delete'])){
          $row = getInfoDelete($_GET['delete'],$_SESSION['_id']);
          echo getModalDelete($row);
          echo "<script type='text/javascript'> $('#modalDelete').modal('show')</script>";
      }
      if(isset($_GET['move'])){
        $row = getInfoDelete($_GET['move'],$_SESSION['_id']);
        echo getModalMove($row);
          echo "<script type='text/javascript'> $('#modalMove').modal('show')</script>";
      }
    ?>
</body>
